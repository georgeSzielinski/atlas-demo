"""Migration 001: baseline Atlas schema.

The complete schema as created by ``setup_database()`` at the time versioned
migrations were introduced: all 32 tables, moved verbatim (CREATE TABLE
IF NOT EXISTS) so this migration is idempotent and additive. On a fresh
database it builds the full current schema; on an existing database every
statement is a no-op and the run simply stamps version 1 in the ledger,
preserving all data.
...
TABLES = (
    ...
    "research_strategy_results",
    "research_provider_results",
    "research_attributions",
    "discoveries",
